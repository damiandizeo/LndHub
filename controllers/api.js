...
redis.monitor(function (err, monitor) {
  monitor.on('monitor', function (time, args, source, database) {// console.log('REDIS', JSON.stringify(args));
  });
});
...
let identity_pubkey = false; // ###################### SMOKE TESTS ########################
...
lightning.getInfo({}, function (err, info) {
  ...
  if (info) {
    ...
    if (!info.synced_to_chain && !config.forceStart) {
      console.error('lnd not synced'); // process.exit(4);
    }
...
const subscribeInvoicesCallCallback = async function (response) {
  if (response.state === 'SETTLED') {
    ...
    let lock = new _class.Lock(redis, 'groundcontrol_hash_' + LightningInvoiceSettledNotification.hash);

    if (!(await lock.obtainLock())) {
      return;
    }

    let invoice = new _class.Invo(redis, bitcoinclient, lightning);
    ...
    const user = new _class.User(redis, bitcoinclient, lightning);
    ...
    if (baseURI) {
      const _api = new _frisbee.default({
        baseURI: baseURI
      });

      const apiResponse = await _api.post('/lightningInvoiceGotSettled', Object.assign({}, {
        headers: {
...
subscribeInvoicesCall.on('status', function (status) {// The current status of the stream.
});
subscribeInvoicesCall.on('end', function () {// The server has closed the stream.
});
...
router.post('/create', postLimiter, async function (req, res) {
  logger.log('/create', [req.id]); // Valid if the partnerid isn't there or is a string (same with accounttype)

  if (!((!req.body.partnerid || typeof req.body.partnerid === 'string' || req.body.partnerid instanceof String) && (!req.body.accounttype || typeof req.body.accounttype === 'string' || req.body.accounttype instanceof String))) return errorBadArguments(res);
  if (config.sunset) return errorSunset(res);
  let u = new _class.User(redis, bitcoinclient, lightning);
  await u.create(req.body.userid, req.body.login, req.body.password);
...
router.post('/addinvoice', postLimiter, async function (req, res) {
  ...
  if (!req.body.amt ||
  /*stupid NaN*/
  !(req.body.amt > 0)) return errorBadArguments(res);
...
router.post('/payinvoice', async function (req, res) {
  ...
  let lock = new _class.Lock(redis, 'invoice_paying_for_' + u.getUserId());
  ...
  lightning.decodePayReq({
    ...
    if (userBalance >= +info.num_satoshis + Math.floor(info.num_satoshis * forwardFee) + 1) {
      ...
      if (identity_pubkey === info.destination) {
        ...
        let UserPayee = new _class.User(redis, bitcoinclient, lightning);
        UserPayee._userid = userid_payee; // hacky, fixme

        await UserPayee.clearBalanceCache(); // sender spent his balance:
        ...
        await invoice.markAsPaidInDatabase(); // now, faking LND callback about invoice paid:

        const preimage = await invoice.getPreimage(); // if (preimage) {
        ...
        return res.send({
          ...
          // amt is used only for 'tip' invoices
          fee_limit: {
            fixed: Math.floor(info.num_satoshis * forwardFee) + 1
          }
        });
      ...
      call.on('data', async function (payment) {
        ...
        if (payment && payment.payment_route && payment.payment_route.total_amt_msat) {
          let PaymentShallow = new _class.Paym(false, false, false);
...
router.get('/gettxs', async function (req, res) {
  ...
  try {
    ...
    for (let locked of lockedPayments) {
      txs.push({
        type: 'paid_invoice',
        fee: Math.floor(locked.amount * forwardFee)
        /* feelimit */
        ,
        value: locked.amount + Math.floor(locked.amount * forwardFee)
        /* feelimit */
        ,
...
router.get('/checkrouteinvoice', async function (req, res) {
  ...
  if (!req.query.invoice) return errorGeneralServerError(res); // at the momment does nothing.
