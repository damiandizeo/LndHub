...
const config = require('../config'); // static cache:
...
class User {
  ...
  async generateAddress() {
    let lock = new _Lock.Lock(this._redis, 'generating_address_' + this._userid);
  ...
  async getCalculatedBalance() {
    ...
    for (let paym of lockedPayments) {
      ...
      /* feelimit */
      Math.floor(paym.amount * forwardFee);
    }
  ...
  async saveUserInvoice(doc) {
    ...
    return await this._redis.rpush('userinvoices_for_' + this._userid, JSON.stringify({
      payment_request: doc.payment_request,
      add_index: doc.add_index
    }));
  }
  /**
  ...
  async getUserInvoices(limit) {
    ...
    for (let invoice of range) {
      ...
      invoice.expire_time = 3600 * 24; // ^^^default; will keep for now. if we want to un-hardcode it - it should be among tags (`expire_time`)

      invoice.timestamp = decoded.timestamp * 1000;
  ...
  async getTxs() {
    ...
    for (let invoice of range) {
      ...
      invoice.timestamp = invoice.timestamp * 1000;
  ...
  async _listtransactions() {
    ...
    try {
      ...
      if (config.bitcoind) {
        let txs = await this._bitcoindrpc.request('listtransactions', ['*', 100500, 0, true]); // now, compacting response a bit
      ...
      if (!_listtransactions_cache) return {
        result: []
      };
  ...
  async accountForPosibleTxids() {
    ...
    for (let tx of onchain_txs) {
      ...
      if (!already_imported && tx.category === 'receive') {
        // first, locking...
        let lock = new _Lock.Lock(this._redis, 'importing_' + tx.txid);
        ...
        let userBalance = await this.getCalculatedBalance(); // userBalance += new BigNumber(tx.amount).multipliedBy(100000000).toNumber();
