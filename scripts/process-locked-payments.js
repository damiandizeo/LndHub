...
(async () => {
  ...
  keys = _class.User._shuffle(keys);
  console.log('fetching listPayments...');
  let tempPaym = new _class.Paym(redis, bitcoinclient, lightning);
  let listPayments = await tempPaym.listPayments(); // DEBUG let listPayments = JSON.parse(fs.readFileSync('listpayments.txt').toString('ascii'));
  ...
  for (let key of keys) {
    ...
    for (let lockedPayment of lockedPayments) {
      ...
      let payment = new _class.Paym(redis, bitcoinclient, lightning);
      payment.setInvoice(lockedPayment.pay_req); // first things first:
      ...
      for (let sentPayment of listPayments['payments']) {
        if ((await payment.getPaymentHash()) == sentPayment.payment_hash) {
          ...
          let sendResult = payment.processSendPaymentResponse({
