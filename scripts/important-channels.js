...
lightning.listChannels({}, function (err, response) {
  ...
  for (let channel of lightningListChannels.channels) {
    if (channel.capacity <= 1000000) {
      console.log('lncli closechannel', channel.channel_point.replace(':', ' '), !channel.active && '--force' || '', '; sleep 10 #', 'low capacity channel', channel.capacity / 100000000, 'btc');
    }
  }
    ...
    for (const important of Object.keys(important_channels)) {
      for (let channel of lightningListChannels.channels) {
        if (channel.remote_pubkey === important && !channel.active && !doneReconnect[channel.remote_pubkey]) {
          ...
          console.log('lncli disconnect', channel.remote_pubkey, '; sleep 5;', 'lncli connect', important_channels[channel.remote_pubkey].uri, '#', important_channels[channel.remote_pubkey].name);
        }
      }
    }
  }
  ...
  if (process.argv.includes('--open')) {
    ...
    for (const important of Object.keys(important_channels)) {
      ...
      if (!atLeastOneChannelIsSufficientCapacity) {
        console.log('lncli disconnect', important, '; sleep 3;', 'lncli  openchannel --node_key', important, '--connect', important_channels[important].uri.split('@')[1], '--local_amt', important_channels[important].wumbo ? '100000000' : '16777215', '--remote_csv_delay 144', '--sat_per_byte 10', '#', important_channels[important].name);
      }
    }
  }
